...
const App: React.FC = () => {
  ...
  const handleSequence = async (tracks: Track[]) => {
    ...
    try {
      ...
      console.error(e);
      setError("Failed to sequence album. Please check your API key or try again.");
  ...
  const getStepNumber = () => {
    switch(appState) {
      case AppState.INPUT_DETAILS: return 1;
      case AppState.INPUT_TRACKS: return 2;
      case AppState.PROCESSING: return 3;
      case AppState.RESULTS: return 4;
      default: return 1;
    }
  };
          ...
          {process.env.API_KEY ? (
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-red-950/20 rounded-full border border-red-900/30">
              <div className="w-1.5 h-1.5 rounded-full bg-red-500"></div>
               <span className="text-[10px] font-medium text-red-500 uppercase tracking-wider">No API Key</span>
            </div>
          )}
        </div>
      </header>
      ...
      <main className="flex-grow w-full max-w-5xl mx-auto px-6 py-12 flex flex-col items-center relative z-10">
        ...
        {appState === AppState.PROCESSING && (
          <div className="text-center flex flex-col items-center justify-center py-20 animate-fade-in">
            ...
            <h2 className="text-2xl font-medium text-white mb-2 tracking-tight">Analyzing Structure</h2>
            <p className="text-zinc-500 max-w-md mx-auto text-sm font-light">
              Processing metadata, calculating optimal flow, and structuring the narrative arc for "{albumData.title}".
            </p>
          </div>
        )}
        ...
        <div className="w-full mt-12 flex flex-col items-center animate-fade-in pb-4">
          <div className="w-full max-w-[728px] h-[90px] bg-[#09090b] border border-zinc-800 rounded flex items-center justify-center relative overflow-hidden">
              {/* Placeholder Visuals */}
              <div className="absolute inset-0 opacity-20" style={{ backgroundImage: 'radial-gradient(#27272a 1px, transparent 1px)', backgroundSize: '8px 8px' }}></div>
              <div className="z-10 flex flex-col items-center text-zinc-700 gap-1">
                  <span className="text-xs font-mono uppercase tracking-widest">Google AdSense</span>
                  <span className="text-[9px]">Space Reserved</span>
              </div>
              {/* 
                  Integration Note: 
                  Paste your AdSense <ins> tag here.
                  Ensure the ad script is loaded in index.html
              */}
          </div>
          <span className="mt-2 text-[9px] text-zinc-700 uppercase tracking-widest">Advertisement</span>
        </div>
